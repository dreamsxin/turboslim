#include "callableresolverawaretrait.h"
...
int CallableResolverAwareTrait_resolveCallable(zval* return_value, zval* this_ptr, zval* callable)
{
...
PHP_METHOD(TurboSlim_CallableResolverAwareTrait, resolveCallable)
{
    ...
    CallableResolverAwareTrait_resolveCallable(return_value, this_ptr, callable);
}
...
static const zend_function_entry fe_TurboSlim_CallableResolverAwareTrait[] = {
    PHP_ME(TurboSlim_CallableResolverAwareTrait, resolveCallable, arginfo_resolveCallable, ZEND_ACC_PROTECTED)
    PHP_FE_END
};

int init_callable_resolver_aware_trait()
{
    zend_class_entry ce;
